...
$('document').ready(function () {
    ...
    // BRING BALLONS TO CENTER (HAPPY BIRTHDAY)
    ...
    // STORY MESSAGE + SHOW FINAL IMAGE
    $('#story').click(function () {
        ...
        let i = 1; // start from the FIRST <p>

        function msgLoop(i) {

            // Hide previous message
            if (i > 1) {
                $("p:nth-child(" + (i - 1) + ")").fadeOut('slow');
            }

            // Show current message
            $("p:nth-child(" + i + ")").fadeIn('slow').delay(1500).promise().done(function () {

                // END OF STORY → Show final image
                if (i >= $('.message p').length) {
                    setTimeout(function () {
                        $('#final-image').fadeIn(1500);
                    }, 1500);
                } else {
                    msgLoop(i + 1);
                }
